...
class WavRollSparseDataset(Dataset):
    def __init__(self, root_dir, window_size):
        ...
        for folder in self.subfolders:
            ...
            fft_path = os.path.join(folder, "Spectrogram.npy")
    ...
    def __getitem__(self, idx):
        item = self.data[idx]
        piano_roll_sp = item['piano_roll_sp']  # sparse matrix [T_piano_roll, 128]
        fft = item['fft']                      # [n_bins, T_fft]

        n_bins, T_fft = fft.shape
        T_piano_roll = item['length']

        # Safety: if too short
        if T_fft < self.window_size:
            dense_roll = torch.from_numpy(piano_roll_sp.toarray()).float()
            fft_window = torch.from_numpy(fft).float()
            return fft_window, dense_roll

        # 1. Sample window in FFT
        start_idx = np.random.randint(0, T_fft - self.window_size + 1)
        end_idx = start_idx + self.window_size
        fft_window = torch.from_numpy(fft[:, start_idx:end_idx]).float()  # [n_bins, window_size]

        # 2. Map FFT window to piano roll indices
        factor = T_piano_roll / T_fft
        piano_start = int(start_idx * factor)
        piano_end = int(end_idx * factor)

        # 3. Slice sparse piano roll before densifying
        piano_slice_sp = piano_roll_sp[piano_start:piano_end, :]  # sparse slice
        piano_slice_dense = torch.from_numpy(piano_slice_sp.toarray()).float()  # to dense [slice_length, 128]

        # 4. Downsample slice to window_size
        slice_length = piano_slice_dense.shape[0]
        if slice_length == self.window_size:
            piano_roll_downsampled = piano_slice_dense
        else:
            downsample_indices = torch.linspace(0, slice_length - 1, steps=self.window_size).long()
            piano_roll_downsampled = piano_slice_dense[downsample_indices, :]  # [window_size, 128]

        return fft_window, piano_roll_downsampled
...
def train_model(
    ...
    eval_interval=50,
    train_split=0.8,
    root_dir="/workspace/src/with_spectogram"
):
    model.to(device)

    dataset = WavRollSparseDataset(root_dir=root_dir, window_size=500)
