"""
Training script for raw audio piano transcription model.
Uses raw waveform snippets instead of spectrograms.
"""
...
from model_raw import RawAudioPianoTranscriptionModel
from dataset_raw import RawAudioSnippetDataset
from utils import (
    get_next_run_folder,
    compute_metrics,
...
def create_datasets_and_loaders(data_dir, config):
    """Create train and validation datasets and dataloaders for raw audio."""
    # Get all files
    ...
    # Create datasets
    train_dataset = RawAudioSnippetDataset(
        data_dir,
    ...
    val_dataset = RawAudioSnippetDataset(
        data_dir,
        config['snippet_frames'],
        snippets_per_file=config.get('snippets_per_file', 20),
...
def train_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for audio_frames, piano_rolls in tqdm(dataloader, desc="Training"):
...
def validate(model, dataloader, criterion, device):
    ...
    with torch.no_grad():
        for audio_frames, piano_rolls in tqdm(dataloader, desc="Validating"):
            audio_frames = audio_frames.to(device)
            piano_rolls = piano_rolls.to(device)
...
def train(data_dir, run_folder, config):
    ...
    # Create model for raw audio
    model = RawAudioPianoTranscriptionModel(
        context_samples=config['n_fft'],  # Context window size
    ...
    print("Architecture: 1D CNN (raw audio) + Transformer (temporal)\n")
    ...
    # Checkpoint paths
    model_path = os.path.join(run_folder, "model_raw.pth")
...
if __name__ == "__main__":
    data_dir = "./processed_data_raw"  # New data directory for raw audio
    run_folder = get_next_run_folder()
    
    print("="*80)
    print("RAW AUDIO PIANO TRANSCRIPTION TRAINING")
    print("="*80)
    print(f"Training run: {run_folder}")
    print(f"Data directory: {data_dir}")
    print(f"Model: 1D CNN on raw audio + Transformer")
    print("="*80 + "\n")
